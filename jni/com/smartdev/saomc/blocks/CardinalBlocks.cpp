...
#include "com/mojang/minecraftpe/world/item/AuxDataBlockItem.h"
...
void CardinalBlocks::registerAuxDataBlock(Block* block) {
	block->init();
	Block::mBlocks[block->blockId] = block;
	Block::mOwnedBlocks.emplace_back(std::unique_ptr<Block>(block));
	Item::mItems[block->blockId] = new AuxDataBlockItem(block->getDescriptionId(), block->blockId - 0x100, block);	
}
